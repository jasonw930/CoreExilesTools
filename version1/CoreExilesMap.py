...
class CoreExilesMap:
    ...
    def dist_and_path(self, start, end):
        if start + ":" + end in self.d_and_p:
            d, p = self.d_and_p[start + ":" + end]
            return d, p
        dist = {start: 0}
        parent = {}
        q = [start]
        ...
        while cur != start:
            path.append(cur)
            cur = parent[cur]
        self.d_and_p[start + ":" + end] = [dist[end], list(reversed(path))]
    ...
    def haul_cost(self, start, dests, planets, final_location):
        fuel = 0
        credits = 0
        for i in range(len(dests)):
            f, p = self.dist_and_path(start if i == 0 else dests[i-1], dests[i])
            fuel += f + 1 + len(planets[dests[i]])
            credits += 50 * (1 + len(planets[dests[i]]))
        a_nearness, a_dist, a_path, a_planet, a_location, a_cost = self.nearest_ashar(dests[-1], planets[dests[-1]][-1], final_location)
        if a_nearness == 1:
            credits += a_cost
        elif a_nearness == 2:
            fuel += 1
